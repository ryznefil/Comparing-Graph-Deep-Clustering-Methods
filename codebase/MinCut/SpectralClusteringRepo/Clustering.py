...
tunables = OrderedDict([
    ('dataset', ['custom_mustafa']),  # 'cora', 'citeseer', 'pubmed', 'cloud', or 'synth'
    ('method', ['mincut_pool', 'diff_pool']),  # 'mincut_pool', 'diff_pool'
    ('H_', [None]),
    ('n_channels', [32]),
    ('learning_rate', [5e-4])
])
...
for T in product_dict(tunables):
    ...
    if P['dataset'] == 'synth':
      ...
      X = np.vstack([clustering_coeff, pagerank]).T
